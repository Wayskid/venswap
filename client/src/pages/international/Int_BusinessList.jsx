...
import { useGetIntBusinessesQuery } from "../../services/appApi";
...
import Int_BusinessListResults from "./Int_BusinessListResults";

export default function Int_BusinessList() {
    ...
    navigate(
      `?search=${search
        ...
        .replace(
          /^\s+|\s+$/g,
          ""
        )}&category=${category}&country=${country}&property=${property}&date_filter=${date_filter}&sort_price=${sort_price}&limit=${limit}&page=${page}`
    );
  }
  ...
  useEffect(() => {
    ...
    setShowFilter(false);
  }, [category, country, property, date_filter, sort_price, limit, page]);
  ...
  return (
    <section className="bg-White">
      ...
      <div className="w-[min(92rem,100%)] mx-auto px-4 md:px-12 lg:px-32 pt-10 pb-16">
        <p className="mb-5 text-3xl">Search Businesses Abroad</p>
              ...
              {country.length > 0 && (
                <div className="">
                  ...
                  <label
                    ...
                    className="grid grid-cols-[1fr_0.15fr] justify-between items-center cursor-pointer peer-checked/accordion:[&>*:last-child]:rotate-[45deg]"
                  >
                    <p className="text-sm">Selected Locations</p>
                    <PiPlusThin className="text-2xl justify-self-end transition-all" />
                  </label>
                  <div className="grid grid-rows-[0] overflow-hidden peer-checked/accordion:grid-rows-1">
                    <div className="mt-1">
                      {country?.split(",").map((coun, index) => (
                        <div
                          key={index}
                          className="grid grid-cols-[1fr_auto] py-[5px] items-center"
                        >
                          <div className="border rounded-sm flex items-center gap-4 relative cursor-pointer p-[3px] mr-[4.3px]">
                            <input
                              ...
                              name={coun}
                              value={coun}
                              checked={country.includes(coun)}
                              onChange={(e) =>
                                handleChange(e, country, setCountry)
                              }
                            />
                            <div className="p-[4.5px] z-20 peer-checked/radio:bg-Orange grid place-items-center transition rounded-xs"></div>
                          </div>
